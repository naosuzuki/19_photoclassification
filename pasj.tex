...
\begin{abstract}
The advancement of technology has brought a rapid increase of supernova discoveries. Subaru Hyper Suprime-Cam (HSC) Transient survey, conducted from Fall 2016 through Spring 2017, yielded 1824 supernova candidates. We are in need of fast type classification for spectroscopic follow-up and developed a machine learning algorithm using Deep Neural Network (DNN) with Highway layers. 
This machine is trained by actual observed cadence and filter combinations so that we can directly input observed data array into machine without any interpretation. We test our model with a data set from LSST classification challenge (Deep Drilling Field only).
Our classifier scores AUC of 0.996 for binary classification (SN~Ia or non-Ia) and 95.3\% in accuracy for three-class classification (SN~Ia, SN~Ibc and SN~II). When we apply our binary classification to HSC Transient data, AUC score becomes 0.925. With two weeks of HSC data since the first detection, this classifier achieves 78.1\% in accuracy for binary classification, and it increases to 84.2\% with full data set. The potential use of machine learning is being discussed.
\end{abstract}
...
We handle missing data by preparing multiple classifiers.
...
Therefore, as shown in table \ref{tab:class_flag}, we can classify 99.3\% of SNe using only five classifiers with different input schedule cases, including those for SNe data containing missing data.
