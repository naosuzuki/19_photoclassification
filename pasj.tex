...
Deep Neural Network (DNN) is now being used for astronomical imaging data for finding strong lens system \citep{petrillo17a} or for galaxy morphology classifications (cite).  Now, machine learning is being introduced for transient survey for detection (cite) and classification \citep{charnock17a}.
...
We conducted a transient survey with Hyper Suprime-Cam on Subaru Telescope which enables us to probe wide filed (1.77 deg$^2$ Field of View) and deep space (26th mag in $i-$band / epoch).  Our primary scientific goals are Type Ia Supernova (SNIa) cosmology, Type II Supernova Cosmology, Super Luminous Supernova (SLSN) studies.   
For this HSC transient survey, 
As is reported in Yasuda et al. (2019), more than 1800 SNe are discovered in a 6-month campaign. 
We deployed AUC boosting for transient detection
In \citet{morii16a}, we have already adopted AUC boosting method for transient detection, namely 'real' vs. 'bogus'.
In \citet{kimura17a}, we use DNN for supernova type classification from 2 dimentional image, and highway block is introduced for
