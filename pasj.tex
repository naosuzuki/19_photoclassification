...
%
\section{Data}
\subsection{Simulatied Data}
We are in need of simulating observed photometric data to train machine.  
We use Subaru/HSC filter responses including system throughput from \citet{kawanomoto18a}.
Throughout this paper, we use $\L$CDM cosmology with $O_m$=0.3 and h=0.7 (TBC).
For normal SN~Ia, we use SALT-II (2.4) \citep{guy10a} model which requires 2 input parameters:
c for color and x1 for stretch.
We adopt the asymetric gaussian distribution of c, and x1 from Mosher et al., and generated
light curves and photometric data points. (to be continued)

To be continued (how to generate erroed data)
%We adopted spectral time series from \citet{kessler19b}. and 
