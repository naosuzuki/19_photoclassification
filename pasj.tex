...
For the precision cosmology, it is very important to keep the purity of Type Ia supernova (SN~Ia) while having uniform sampling from bright to faint SNe~Ia.  Spectroscopic follow-up has been essential to distinguish faint SN~Ia from SN~Ibc which has a similar light curve behavior \citep{scolnic14a}.   A large amount of precious telescope time is dedicated to the follow-up programs, and we would like to make an efficient use of those telescopes.
...
The light curves of SN~Ib and SN~Ic are similar to that of SN~Ia and it always contaminates SN~Ia cosmology program,
and togheter we call them SN~Ibc.  They are fainter than SN~Ia and redder in general.  
...
For training, 370,345 light curves are generated for LSST Deep Drilling Field (DDF), and 514,954 for 
HSC observed data.
We dropped ones which has less than 3$\sigma$ detection at the maximum since our detection criterion
is 5$\simga$.  %Prior to the modeling, we did the convergence test
...
\begin{figure}[ht]
  ...
  \end{center}
  \caption{%
  Confusion matrix in three-class classification of PLAsTiCC test set. The input to the classifier is absolute magnitude and normalized flux. The numbers in parentheses represent the raw numbers.
  }%
...
\begin{figure*}[ht]
    ...
    \vspace{2mm}
    \caption{%
  ROC curves and precision Recall curves in two-class classification of light curve verified HSC supernovae. 
  The input to the classifier is absolute magnitude and normalized flux.
  Each colored line represents the performance for each of the five classifiers with different input cases, and the that for all of their outputs.
}%
...
\hline
Index &    ID &  Case &        z &  z\_src$^*$ &  \multicolumn{3}{p{10.0em}}{SALT2 fitting} &\multicolumn{5}{|p{14.5em}}{Classifier (Input$^{**}$: $M+f$)} & \multicolumn{5}{|p{12.0em}}{Classifier (Input$^{**}$: $m+f$)}\\
\hline
...
\hline
0    &  aaau &     1 &    $0.370_{-0.072}^{+0.110}$ &         3 &    7 &    Ia? &   incomplete &    0.399 &    0.414 &    0.041 &    0.545 &      II &    0.401 &    0.447 &    0.013 &    0.541 &      II \\
1    &  aaav &     1 &    $3.280_{-2.423}^{+0.167}$ &         4 &   17 &  nonIa &     OK &    0.157 &    0.074 &    0.001 &    0.925 &      II &    0.269 &    0.314 &    0.168 &    0.518 &      II \\
2    &  aabj &     0 &    $0.361_{-0.008}^{+0.007}$ &         2 &    8 &  nonIa &   incomplete &    0.641 &    0.680 &    0.010 &    0.310 &      Ia &    0.574 &    0.583 &    0.020 &    0.396 &      Ia \\
...
\begin{figure*}[ht]
    \begin{tabular}{cc}
        ...
        \end{minipage}
    \end{tabular}  \caption{%
    Confusion matrices in the two-class classification of the PLAsTiCC 1st classifier (left) and our best classifier given absolute magnitude and normalized flux (right).
    }%
