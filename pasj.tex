...
the binary classification AUC score is 0.921 for supernovae labeled by SALT2 fit.
...
HSC-SSP Transient Survey is composed of two seasons, and the first season is executed for a consective 6 months from November 2016 through April 2017.
...
Our target, high-redshift SN~Ia, is faint ($\sim$ 24th mag in $i-$band) for spectroscopic identification even with the powerful large telescopes: GMOS Gemini \citep{hook04a}, GTC, Keck LRIS \citep{oke95a}, VLT FORS, Subaru FOCAS \citep{kashikawa02a} and AAT 2dF Spectrograph.  
...
Observations of the early phase Core Collapse Supernova provides us with a crucial information on the size of the progenitors \citep{thompson03a,chevalier11a}.  
...
SN~Ib, SN~Ic and SN~II are all core collapose supernovae and classified by the spectral features \citep{filippenko97a}.
The light curves of SN~Ib and SN~Ic are similar to that of SN~Ia and it always contaminates SN~Ia cosmology program, and togheter we call them SN~Ibc.  They are fainter than SN~Ia and redder in general.  
...
To evaluate our machine, we need a dataset which is labed by true identities. 
...
and they simulated a realistic photometric data set for LSST.
Inside the PLAsTiCC data, simulated data for Deep Drilling Field is the most similar data set to 
...
The discovery of 1842 transients from a six-month HSC-SSP Transient Survey in the period of
...
\section{Testing DNN Model with PLAsTiCC data} 
%
...
Before we apply our model to the HSC observed data, we test it with LSST simulated
classification challenge data set, PLAsTiCC \citep{allam18a} which is composed of realistic
photometric data set with errors on time-variable objects.
To evaluate our model, we are in need of data set with labels of true identity. 
PLAsTiCC DDF data set is similar to HSC-SSP Transient Survey, and we took advantage of it.
...
provided because we knew that the number of training data set is not good enough to
...
The simulated light curve has a class ratio of SN~Ia:Ibc:II=0.60: 0.06:0.34, and the positions of these peaks are randomly shifted between 450 days.
...
The classifier given absolute magnitude and normalized flux performed best in both two-class and three-class in the simulated data classification.
...
\end{comment}
%
\subsection{PLAsTiCC data for Validation}
%\begin{itemize}
...
For test data set, we extract 2,297 light curves from PLAsTiCC data set, and applied our model to it.
...
We find that the combination of max-normalized flux ($f$) and pseudo-absolute magnitude ($M$) performs
best, although the information is reduntant, we suspect the different distribution of the data gives extra clue to the machine.
...
\begin{figure}[ht]
  ...
  \end{center}
  \caption{%
  Normalized confusion matrix in three-class classification of PLAsTiCC test set. The input to the classifier is absolute magnitude and normalized flux. The proportions in each row sum to 1. The numbers in parentheses represent the raw numbers.
  }%
...
As described in the HSC transient servey overview paper \citep{yasuda19a}, this survey is part of HSC SSP survey, where observations were performed in two areas with different observation frequencies, "Deep" area and "Ultra-Deep" area.
...
The spectral redshift (spec-z) information are adopted from the results of the follow-up spectrum observation by AAT/AAOmega performed in 2018 and that from DEIMOS \citep{DEIMOS2018}, FMOS-COSMOS \citep{FMOS-COSMOS2015}, C3R2 \citep{C3R2_2017}, PRIMUS \citep{PRIMUS2011} and COSMOS catalogs.
...
In the three-class classification of the PLAsTiCC dataset, we found that the accuracy of Ibc was lower than that of other classes.
...
Therefore, we discuss the transition of performance against the number of input.
...
In addition, the number of follow-up supernovae classified by the classifier can be estimated from the cumulative number in Figure \ref{fig:lcps}.
...
of cosmic evolution \citep{fukugita04a}. 
...
Lick Observatory Supernova Survey \citep{li11a} reports the relative ratio between SN~Ia:Ibc:II to 
...
z$\sim$0.5. 
The reason why SN~II fraction does not go to zero at z$\sim$1 in Figure \ref{fig:hsc3_type_frac_alongz} is 
