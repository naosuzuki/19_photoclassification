...
%DIF DEL pasj.tex                            Mon Jul 27 10:45:30 2020
...
\let\square\relax
%DIF 7d7
%DIF < %\documentclass[useamsfonts]{pasj01}
%DIF -------
\usepackage{amsfonts}
\usepackage{graphicx}
%DIF 10-12d9
...
%DIF < %% \SetRunningHead{Astronomical Society of Japan}{Usage of \texttt{pasj00.cls}}
...
\DIFdelbegin %DIFDELCMD < \email{ichiro.takahashi@astr.tohoku.ac.jp}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \email{ichiro.takahashi@ipmu.jp}
\DIFaddend 
...
%
\begin{abstract}
The advancement of technology has resulted in a rapid increase in supernova discoveries. The Subaru \DIFdelbegin \DIFdel{/}\DIFdelend Hyper Suprime-Cam (HSC) \DIFdelbegin \DIFdel{transient survey}\DIFdelend \DIFaddbegin \DIFadd{Transient Survey}\DIFaddend , conducted from fall 2016 through spring 2017, yielded 1824 supernova candidates. This gave rise to the need for fast type classification for spectroscopic follow-up and prompted us to develop a \DIFdelbegin \DIFdel{machine learning }\DIFdelend \DIFaddbegin \DIFadd{machine-learning }\DIFaddend algorithm using a deep neural network (DNN) with highway layers. 
This machine is trained by actual observed cadence and filter combinations such that we can directly input the observed data array into the machine without any interpretation. We tested our model with a dataset from the LSST classification challenge (Deep Drilling Field only).
Our classifier scores an AUC of 0.996 for binary classification (SN~Ia or non-Ia) and 95.3\% accuracy for three-class classification (SN~Ia, SN~Ibc, or SN~II). Application of our binary classification to HSC transient data yields an AUC score of 0.925. With two weeks of HSC data since the first detection, this classifier achieves 78.1\% accuracy for binary classification, and the accuracy increases to 84.2\% with the full dataset. This paper discusses the potential use of machine learning for supernova type classification purposes.
\end{abstract}
...
For precision cosmology, it is highly important to maintain the purity of \DIFdelbegin \DIFdel{Type Ia supernovae (SNe}\DIFdelend \DIFaddbegin \DIFadd{a Type Ia supernova (SN}\DIFaddend ~Ia) while performing uniform sampling of these \DIFdelbegin \DIFdel{SNe }\DIFdelend \DIFaddbegin \DIFadd{supernovae (SNe~Ia) }\DIFaddend ranging from bright to faint. Spectroscopic follow-up has been essential to distinguish a faint SN~Ia from a Type Ib supernova (SN~Ib) and a Type Ic supernova (SN~Ic)\DIFaddbegin \DIFadd{, }\DIFaddend which have similar light curve behavior \citep{scolnic14a}. Considering that a large amount of precious telescope time is dedicated to these follow-up programs, it is desirable to make efficient use of these telescopes.

The scale of surveys is becoming larger, and it has become impossible to trigger spectroscopic follow-up for all of the candidates in real time. It has therefore become necessary to develop a new classification scheme, and a natural path along which to proceed would be to perform photometric classification \citep{sako11a,jonesl8a}. The rise of \DIFdelbegin \DIFdel{machine learning }\DIFdelend \DIFaddbegin \DIFadd{machine-learning }\DIFaddend technologies has resulted in astronomical big data commonly being analyzed by using \DIFdelbegin \DIFdel{machine learning }\DIFdelend \DIFaddbegin \DIFadd{machine-learning }\DIFaddend techniques.   

Neural networks have been used for photometric redshift studies from the early stages \DIFdelbegin \DIFdel{. 
Today, many Machine Learning methods are applied to the photometric redshift studies }\DIFdelend \DIFaddbegin \DIFadd{and these studies often involve the use of machine-learning methods }\DIFaddend \citep{collister04a,carliles10a,pasquet19a}.
Deep neural networks (DNNs) are being used to process imaging data to identify strong lens systems \citep{petrillo17a} or for galaxy morphology classifications \citep{hausen19a}. Currently, machine learning is being introduced to process transient survey data for detection \citep{goldstein15a} and classification purposes \citep{charnock17a}.
...
\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{Previously }\DIFaddend \citet{Kimura17}, we adopted a DNN for SN type classification from a two-dimensional image, and highway block was introduced for the optimization of layers.
...
\label{sec:tasks}
The Subaru HSC Transient Survey forms part of the Subaru Strategic Project (SSP), which is a five-year program that includes a total of 300 dark nights \citep{aihara18a,miyazaki18a}.
...
The HSC is mounted on \DIFdelbegin \DIFdel{the }\DIFdelend prime focus for two weeks in dark time.   
Weather permitted, we aimed to observe two data points per filter per lunation.
Details of the survey strategies and the observing logs were reported \citet{yasuda19a}, and we used the \DIFdelbegin \DIFdel{observed photometric data described in \citet{yasuda19a} }\DIFdelend \DIFaddbegin \DIFadd{reported photometric data \citet{yasuda19a} to develop the machine-learning method}\DIFaddend .

One of our primary goals with the HSC-SSP Transient Survey is SN~Ia Cosmology\DIFaddbegin \DIFadd{, }\DIFaddend which aims to perform the most precise measurement of dark energy at high-redshift and test whether the dark energy varies with time \citep{linder03b}.
...
Our HST program uses non-disrupted ToO, which means we are required to send the request for observation latest by Wednesday, two weeks prior to the observation.
...
Our target, high-redshift SN~Ia, is faint ($\sim$ 24th mag in the $i-$band) for spectroscopic identification even with the most powerful large telescopes: GMOS Gemini \citep{hook04a}, GTC, Keck LRIS \citep{oke95a}, VLT FORS, Subaru FOCAS \citep{kashikawa02a}\DIFaddbegin \DIFadd{, }\DIFaddend and AAT AAOMega Spectrograph.  
Thus, it is critical to hit the target at the time of maximum brightness either by using ToO (GTC), queue mode (VLT)\DIFaddbegin \DIFadd{, }\DIFaddend or classical scheduled observation (Keck, Subaru, ATT).

\DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend SN~Ia requires approximately 18 days from its explosion to reach its maximum in rest frame \citep{conley06a,papadogiannakis19a}. %DIF < Given the fact, we are looking at high-redshift SN~Ia (z$>$1), we have about a month in observed frame for a SN~Ia from explosion to the maximum. 
Given this fact, the supernovae with which we are concerned is high-redshift SN~Ia (z$>$1), in which case we have approximately one month in the observed frame for \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend SN~Ia from the time of explosion until it reaches its maximum. However, our task is to identify these supernovae two weeks prior to the maximum, which means we have only \DIFdelbegin \DIFdel{two }\DIFdelend \DIFaddbegin \DIFadd{2 }\DIFaddend data points per filter. In addition to that, the sky conditions continue to change, and we may not have the data as originally planned. In reality, our identification has to proceed despite data points being missing on the rise.
...
Specifically, the SN~II cosmology program requires timely spectroscopic follow-up to measure the \DIFdelbegin \DIFdel{expansion velocity of photosphere }\DIFdelend \DIFaddbegin \DIFadd{velocity at which the photosphere is expanding }\DIFaddend from the H$\beta$ line \citep{dejaeger17a}.
%DIF < SLSN is of great interest today, it is relatively a rare event\citep{quimby11a} and its mechanism has quite a diversity \citep{galyam12a,Moriya18SLSN}, and it can probe high redshift universe \citep{cooke12a}. 
\DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend SLSN is of great interest today, because it is a relatively rare event\citep{quimby11a}\DIFdelbegin \DIFdel{and its mechanism has quite a diversity }\DIFdelend \DIFaddbegin \DIFadd{. SLSN occur via quite a diverse range of mechanisms }\DIFaddend \citep{galyam12a,Moriya18SLSN}, and \DIFdelbegin \DIFdel{it }\DIFdelend can be used to probe the high-redshift universe \citep{cooke12a}.    
For the SLSN project, timely spectroscopic follow-up is also critical \citep{moriya19a,curtin19a}.

An early phase SN~Ia provides us with clues on the explosion mechanism \citep{maeda18a} and progenitors \citep{cao15a}, and it is a popular topic to study.
...
%DIF < Shock Breakout lasts only for a few $\sim$ several hours at the time of explosion and it becomes as luminous as at the time of the maximum \citep{gezari08a}. 
%DIF < Shock Breakout contains the information on projenitor radius \citep{nakar10a}, and HSC is expected to find these candidates as well \citep{tominaga11a}.
%DIF < 
%DIF < Tidal Disruption Event \citep[TDE]{gezari08b,holoien14a} is also expected to be observed by HSC-SSP Transient Survey.  The light curve is similar to
%DIF < the supernova, but it monotonically drops at a specific rate \citep[$t^{-5/3}$]{lodato11a} and the event happens on the core of the galaxy.  
%DIF < We expect to have a few events per season \citep{kochanek16a}.
%DIF < 
%DIF < It is always possible to discover something new when we explore a new parameter space which is the depth for the case
%DIF < of Subaru HSC.   Rapid transients are discovered by HSC, but their identities are yet to be known (Tampo et al.  2020 in prep).  
%DIF < Similar objects are also reported by DES team \citep{pursianinen18a}.
%DIF < 
%DIF < In many science cases, it is essential to have a timely classification at the early phase of transient so that we can trigger follow-up observations.
%DIF < \citep{maeda18a,jiang18a}
%DIF < \citep{moriya19b}
%DIF < 
%DIF < 
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Classification method for HSC-SSP \DIFdelbegin \DIFdel{transient survey}\DIFdelend \DIFaddbegin \DIFadd{Transient Survey}\DIFaddend }
We designed two \DIFdelbegin \DIFdel{machine learning }\DIFdelend \DIFaddbegin \DIFadd{machine-learning }\DIFaddend models with the emphasis on identifying SN~Ia, which requires a time-sensitive trigger for HST IR follow-up.
The first model operates in binary mode and classifies whether a transient is of the SN~Ia \DIFaddbegin \DIFadd{type}\DIFaddend .
...
The light curves of SN~Ib and SN~Ic, which are collectively referred to as SN~Ibc, are similar to those of SN~Ia and always contaminate the SN~Ia cosmology program. They are fainter than SN~Ia and \DIFdelbegin \DIFdel{redder }\DIFdelend \DIFaddbegin \DIFadd{more intensely red }\DIFaddend in general.
...
The discovery of 1824 SNe recorded during the six-month HSC-SSP Transient Survey during the period of
November 2016 through April 2017 was reported and described \DIFdelbegin \DIFdel{in }\DIFdelend \citet{yasuda19a}.
The survey is composed of \DIFaddbegin \DIFadd{data from }\DIFaddend the Ultra-Deep (UD) and Deep layers in the COSMOS \DIFaddbegin \DIFadd{catalog }\DIFaddend \citep{scoville07a}.
The median 5$\sigma$ limiting magnitudes per epoch are 26.4, 26.3, 26.0, 25.6, and 24.6 mag (AB)
for the $g$-, $r2$-, $i2$-, $z$-\DIFaddbegin \DIFadd{, }\DIFaddend and $y$-bands, respectively, for the UD layer. 
For the \DIFdelbegin \DIFdel{Deep }\DIFdelend \DIFaddbegin \DIFadd{deep }\DIFaddend layer, the depth is 0.6 mag shallower.

The SN dataset consists of time series photometric data (flux, magnitude, and their errors) in each band for each SN.
Because part of the $y$-band photometric data contains residuals due to improper background subtraction influenced by scattered light \citep{aihara18dr},
we excluded the $y$-band data from our study, considering the impact thereof on the classification performance.
...
The spectral redshifts (spec-z) are adopted from the results of the follow-up spectrum observations by AAT/AAOmega performed in 2018 and those from the DEIMOS \citep{DEIMOS2018}, FMOS-COSMOS \citep{FMOS-COSMOS2015}, C3R2 \citep{C3R2_2017}, PRIMUS \citep{PRIMUS2011}\DIFaddbegin \DIFadd{, }\DIFaddend and COSMOS catalogs.
For those without spec-z, the photometric redshifts (photo-z) were adopted from the COSMOS catalog and those calculated from the HSC-SSP survey data \citep{HSCSSP_photo-z2018}.
...
We adopted an asymmetric Gaussian distribution for c, and x1 from \citet{mosher14a}, and generated
light curves and simulated photometric data points based on the observation schedule. 
Apart from SN~Ia, we used published spectral time series \DIFdelbegin \DIFdel{from \citet{kessler19b} which contains }\DIFdelend \DIFaddbegin \DIFadd{\citet{kessler19b} that include }\DIFaddend both observed Core-Collapse SN data and light curves from the simulation.
We combined an equal number of SN~Ia and non-SN~Ia observations to approximate the observed fractions. 
%DIF < These fraction does not need to be exact for our purpose of classification, but it is important that the minority is not buried in the majority.
...
\DIFdelbegin \DIFdel{For three class classification, we set the }\DIFdelend \DIFaddbegin \DIFadd{The data were classified into three classes using a }\DIFaddend ratio of SN~Ia:SN~Ibc:SN~II=10:3:7.
...
We used the filter response including system throughput from \DIFaddbegin \DIFadd{a previous study }\DIFaddend \citet{kawanomoto18a}. 
...
%DIF < For HSC simulation, the size of error varies night to night due to sky condition. 
The magnitude of the error \DIFdelbegin \DIFdel{in }\DIFdelend the HSC simulation would have to consider varies from night to night because of sky conditions. 
We measured and derived the flux vs. error relationship from the \DIFdelbegin \DIFdel{actual observed data at simulating epoch }\DIFdelend \DIFaddbegin \DIFadd{data that were actually observed after a number of epochs during the simulation }\DIFaddend and applied that relationship to the simulated photometric data.
%DIF < For LSST simulation, we find the flux-error relation from the Deep Drilling Field (DDF) of the one used in Kaggle Competition \citep{malz19a} and applied that to the simulated photometric data.
...
%DIF < Prior to the modeling, we did the convergence test
\DIFdelbegin \DIFdel{For training}\DIFdelend \DIFaddbegin \DIFadd{To train the DNN}\DIFaddend , we generated 514,954 light curves \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend HSC observed data. 
Their class ratio is SN~Ia:Ibc:II=0.59:0.07:0.34, and their peak timings are randomly shifted by 450 days.
%
%
\begin{figure}[htbp]
  \begin{center}
    %DIF < \includegraphics[width=\columnwidth]{figures/SimLCsamples.eps}
     \includegraphics[width=130mm]{figures/SimLCsamples.eps}
  \end{center}
  \vspace{-6mm}
  \caption{%
  Overlay plots of simulated light curves with z between 0.1 and 1.2.
  Each panel shows the plots of Ia, Ibc, II data from the left, and the $g$-, $r2$-, $i2$-, $z$-, $y$-bands from the bottom.
...
%DIF < Non-Ia light curves are generated based on the PLAsTiCC template and using redshift as a parameter.
%DIF < The number of Ia and non Ia light curves has been adjusted to a ratio of 1:1.
%DIF < The ratio of Ibc and II is set to Ibc:II=3:7 with reference to that of the simulated dataset for PLAsTiCC \citep{plasticc_models}.
%DIF < In other words, the numbers of the three classes have a ratio of Ia:Ibc:II=0.5:0.15:0.35.
%DIF < The selection weights of individual SED time-series models in each of the Ibc and II classes are also matched to those in the PLAsTiCC dataset.
%DIF < The redshift is selected in the range of 0.1 to 2.0, and the event rate depends on the number distribution of galaxies in the COSMOS2015 catalog \citep{2016ApJS..224...24L}.
%DIF < In addition, the timing at which SNs occur is shifted by randomly setting an offset that indicates the difference between the SN phase and the observation schedule.
%DIF < Of all the light curves that are finally created, faint events with a maximum signal-to-noise ratio of less than three are rejected.
%
\subsection{Preprocessing of input data}
\label{sec:preproc}
%DIF < We found that in experiments using simulation data, it is better to be input x follow as,
...
%DIF < where $m_i$ is i-th magnitude and $\mathrm{DM}\left(z\right)$ is distance modulus of the redshift value $z$.
...
%DIF < the calculation of the magnitude was dealt with using Luptitude (Lupton magnitude). %simplified Lupton magnitude 
\begin{eqnarray}
...
%DIF < It has been proven among PLAsTiCC competition that a combination of features can perform very well \citep{boone19a}, but it needs the data augmentation, interpolation or extrapolation to extract features.

The fact that we went one step further by leaving the observed data as raw as possible, means that our input consists of a simple array of magnitudes. An attempt such as this would not have been possible ten years ago; however, owing to advancements in computing and the deep learning technique, this has become reality. Among the many \DIFdelbegin \DIFdel{machine learning }\DIFdelend \DIFaddbegin \DIFadd{machine-learning }\DIFaddend methods, we decided to use a deep neural network to enable us to classify astronomical objects from the raw observed data. 

\subsection{Model design}
\label{sec:model} %added by IT
In this section, we describe the design of our DNN model,
\footnote{The code for our model is available at $\langle$https://github.com/ichiro-takahashi/snclass$\rangle$.}
which accepts an array of observed magnitudes as its input and outputs the SN classification with probabilities. We adopted a highway layer (also known as a ``layer in layer'' \citep{srivastava15a}) as a core part of our network. Compared to plain DNN, the performance of a highway layer improves when the network is deep in terms of parameter optimization \citep{srivastava15b}.   
\DIFdelbegin %DIFDELCMD < 
...
\begin{figure}[htbp]
  \begin{center}
    %DIF < \includegraphics[width=\columnwidth]{figures/model_all.eps}
...
%DIF < {\bf Memo Distance Modulus \& Normalized Flux needs to be written for preprocessing}
%DIF < Based on our experiments (table \ref{tab:p_test}), the combination of absolute magnitude and normalized flux performs best, and therefore, our
...
%DIF < The DNN model receives the total of P brightnesses observed in multiple bands as an input and outputs $C$ values, where $C$ is the number of classes in classification task, and $C = 1$ in regression task.
\DIFdelbegin %DIFDELCMD < 
...
\DIFdel{$\langle$https://github.com/deepmind/sonnet$\rangle$.}} %DIFAUXCMD
...
{\bf Highway layer:}
We adopted a Highway layer \citep{srivastava15a} and optimized the number of layers therein during the hyperparameter search. In theory, it would be possible to design a very deep \DIFaddbegin \DIFadd{highway }\DIFaddend layer with more layers than would be necessary. However, in reality, it is not trivial to optimize the number of layers.   
%DIF < In this study, we adopted Highway Layer \citep{highway} as the basic part of the network structure.
...
%DIF < There are many studies to solve the problem such as optimization, initialization, and network architecture.
A highway layer is a technique that stabilizes the learning process by devising the network structure.
We previously used a highway layer, which we tested on 2D images, and it delivered good performance \DIFdelbegin \DIFdel{\citep{Kimura17}}\DIFdelend \DIFaddbegin \DIFadd{\citet{Kimura17}}\DIFaddend . Details of the use and advantages of the highway layer are provided in \DIFaddbegin \DIFadd{a previous paper of ours }\DIFaddend \citet{Kimura17}. This encouraged us to adopt a highway layer scheme for this analysis.
%DIF < \begin{equation}
%DIF < Highway\left(h\right) = H\left(h\right) \otimes T \left(h\right) + h \otimes C\left(h\right)
%DIF < \end{equation}
%DIF < Highway layer, shown in Fig.\ref{fig:dnn_model}, has multiple paths from the input to the output.
...
%DIF < The two gate functions $T$ and $C$ control how much of the outputs which are transforming the input and carrying it, respectively.
%DIF < One gate functions, $T$, controls the transformation of input and the other gate function, $C$, controls  
...
\begin{equation}
    ...
    \end{array}\right.
\end{equation}
%DIF < We define a highway block which is composed of three layers: Highway Layer, Batch Normalization Layer and Activation Layer.
%DIF < and adjusted the size of the network by changing the number of highway blocks.
%DIF < The highway block is composed of a maximum of three layers including a highway layer as shown in figure \ref{fig:dnn_model}.
%DIF < However, layers other than Highway layer are determined by a hyperparameter search described later.
%DIF < This block itself is laid multiple times.
%DIF < and this number of blocks $N_{bloack}$ is one of the hyperparameters and optimized later. 
%DIF < The optimal structure of the network was determined by hyperparameter search.
...
%DIF < The subjects of hyperparameter search are the number of highway blocks $N_{block}$, the number of hidden layer dimensions $d$ of highway blocks, the dropout ratio, the presence of batch normalization, and the type of activation function.

{\bf Batch \DIFdelbegin \DIFdel{Normalization }\DIFdelend \DIFaddbegin \DIFadd{normalization }\DIFaddend layer:}
We adopt batch normalization \citep{batch_norm} to accelerate and stabilize the optimization. Even if a large number of parameters need to be trained, batch normalization facilitates convergence of the training process, reduces errors on the slope when we apply entropy minimization, prevents the average and dispersion from becoming exponentially large in deep layers, and minimizes the biases on outputs \citep{understanding_batch_norm}.
Batch normalization is effective in many cases. However, the performance of a model that employs both batch normalization and dropout may degrade (\cite{dropout_and_batch_norm}).   

{\bf Activation layer:}
Each neuron is activated by using nonlinear transformation. Nonlinearity is an important component of DNN, because it allows a wide variety of expressions. Note that the majority of the layers, including fully connected layers, involve a linear transformation and, even if a number of layers were to exist, it would be equivalent to one single linear transformation. Thus, nonlinear transformation is essential to allow each neuron the freedom to have any necessary values.
For the first iteration, we do not know what kind of transformation is the best; thus, the transformation itself is taken as one of the hyperparameters. In our work, we used the functions ``tf.nn,'' in {\it Tensorflow}.

{\bf Second \DIFdelbegin \DIFdel{Fully Connected }\DIFdelend \DIFaddbegin \DIFadd{fully connected }\DIFaddend layer:}
After $T$ repetitions of the highway layer, batch normalization layer, and activation layer,
it is necessary to convert the number of neurons to the number of SNe times the number of SN classes. This operation is opposite to that of the first fully connected layer.
...
However, we note that this is a \DIFdelbegin \DIFdel{pseudo-probability }\DIFdelend \DIFaddbegin \DIFadd{pseudoprobability }\DIFaddend and that it differs from the statistical probability.
...
%DIF < By combining them, there are merits that the knowledge obtained in the preliminary experiment can be used and the time required for the search can be shortened.

%DIF <  We optimized the hyperparameters of the DNN model using the library, optuna(\cite{optuna}).
...
%DIF < We trained the DNN model using the hyper parameters obtained by optimization, and evaluated the performance of the DNN model.
%
\begin{table}[htbp]
...
\end{figure}
%
%DIF < In this section, we describe the method used for SN type classification and regression of the value of redshift.
%DIF < Next, we will explain how to learn the model and how to determine the hyperparameters of the model.
%
%DIF < DNN is one of machine learning methods inspired by the biological brain system.
%DIF < DNN combines a large number of elements that mimic neurons, and each element has only a simple function, but it can obtain complex functions as a whole.
%DIF < DNN model can obtain the features needed to solve the target task from the given input without explicitly giving the features.
%DIF < Our goal is to classify the type of the celestial body or to estimate the redshift value from the observed light curve.
%DIF < We used DNN model for them.
%DIF < The DNN model receives the total of P brightnesses observed in multiple bands as an input and outputs $C$ values, where $C$ is the number of classes in classification task, and $C = 1$ in regression task.
%DIF < We used the structural model shown in Fig.\ref{fig:dnn_model}.
%
%DIF < In this study, we adopted Highway Layer \citep{highway} as the basic part of the network structure.
%DIF < The depth and/or layer size of the DNN model are directly related to the complexity of the features that can be obtained, and greatly affect the %performance for the task.
%DIF < However, a too deep model is difficult to learn and rather causes performance degradation.
%DIF < There are many studies to solve the problem, such as optimization, initialization, and network architecture.
%DIF < Highway layer is a technique that stabilizes learning by devising the network structure.
%DIF < 
%DIF < Highway layer, shown in Fig.\ref{fig:dnn_model}, has multiple paths from the input to the output.
%DIF < The output of Highway Layer is calculated from the values of the multiple paths.
%DIF < The output, ${\bf Highway} \left(x\right)$, is formulated as
%DIF < \begin{equation}
%DIF <     {\bf Highway} \left(x\right) = T \left(x\right) \cdot H \left(x\right) + C \left(x\right) \cdot x,
%DIF < \end{equation}
%DIF < where $H$ is a non-linear transformation layer, $T$ is the transformation-gate function layer, and $C$ is the carry-gate function layer.
%DIF < The two gate functions $T$ and $C$ control how much of the outputs which are transforming the input and carrying it, respectively.
%DIF < Highway layer includes other layers inside.
%DIF < This structure is called layer in layer.
%DIF < 
%DIF < We defined the functions as 
%DIF < \begin{eqnarray}
%DIF <     H \left(x\right) &=& \mathrm{a} \left( F \left(x, \left\{W_H, b_H\right\}\right) \right), \\
%DIF <     T \left(x\right) &=& \mathrm{a} \left( F \left(x, \left\{W_T, b_T\right\}\right) \right), \\
%DIF <     C \left(x\right) &=& 1 - T \left(x\right),
%DIF < \end{eqnarray}
%DIF < where $a$ is an activation function, that is,
%DIF < \begin{eqnarray*}
%DIF <     \mathrm{a} \left(z\right) &=& \left( \sigma\left(z_1\right),\sigma\left(z_2\right), \ldots, %\sigma\left(z_d\right) \right)^T, \; z \in \mathbb{R}^d, \\
%DIF <     \sigma \left(z_i\right) &=& \frac{1}{1 + e^{-z_i}},
%DIF < \end{eqnarray*}
%DIF < $d$ is the size of the dimension of the input and the output of Highway Layer, and
%DIF < $F$ is a fully connected layer parameterized by $W \in \mathbb{R}^{d \times d}$ and $b \in \mathbb{R}^d$, is formulated as
%DIF < \begin{equation}
%DIF <     F \left(x, \left\{W,b\right\}\right) = W x + b.
%DIF < \end{equation}
%DIF < Fully connected layer is a popular and basic layer of DNN.
%DIF < This formulation of Highway layer is same as (\cite{highway}).
%DIF < 
%DIF < 
...
%DIF < To add noise to flux, as follows, we replaced $f_i$ with $f_i + \epsilon_i$ in eq.\ref{eq:scaled_flux} and eq.\ref{eq:mag},
%DIF < where $\epsilon_i$ is a random noise drawn from the Gaussian distribution with zero mean and $e_i^2$ variance, and $e_i$ is the flux error corresponding to $f_i$.
...
%DIF < Table\ref{tb:hp_grid} shows the grid search settings, and Table\ref{tb:hp_search} shows the search settings for the TPE algorithm.
%DIF < Table\ref{tb:hp} shows the grid search settings and the search settings for the TPE algorithm.
%DIF <  We determined the optimal DNN model size for classification by hyperparameter search.
%DIF <  The scale for measuring the goodness of the model was the accuracy of the validation data, which was not used for training.
%DIF <  As the first step of hyperparameter search, we performed a grid search for each element created by Cartesian product of the values in table \ref{tb:hp_grid}.
%DIF <  Subsequently, we searched in the range in table \ref{tb:hp_search} using the TPE algorithm.
%
%DIF <  The performance evaluation of the models was performed by 5-fold cross validation.
%DIF <  Let the class corresponding to the element with the highest value among the prediction vector be the class predicted by DNN.
%DIF <  The accuracy is evaluated by the matching rate of DNN predicted class and the ground truth.
%DIF < 

\subsection{Testing the DNN model with PLAsTiCC dataset} 
\label{sec:p}
%
%DIF < We verify the performance of the classifier described in the previous section using the PLAsTiCC dataset \citep{plasticc_dataset}.
Before we applied our model to the data \DIFdelbegin \DIFdel{observed by }\DIFdelend \DIFaddbegin \DIFadd{captured with the }\DIFaddend HSC, we tested it with a dataset resulting from the LSST simulated classification challenge, i.e., the PLAsTiCC dataset \citep{allam18a,malz19a}, which is composed of realistic photometric data with errors on time-variable objects.
To evaluate our model, we required a \DIFdelbegin \DIFdel{dataset with labels of true identity}\DIFdelend \DIFaddbegin \DIFadd{labeled dataset to identify true data}\DIFaddend . 
...
%DIF < For training, 370,345 light curves are generated for LSST DDF.
%DIF < We use filter response including system throughput from \citet{ivezic19a}. 
...
%DIF < , and applied our model to it.
\DIFdelbegin \DIFdel{These light }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend curves are labeled Ia, Ibc, or II, to identify the type of SN each curve represented. The \DIFdelbegin \DIFdel{light }\DIFdelend curves were simulated \DIFdelbegin \DIFdel{to occur }\DIFdelend \DIFaddbegin \DIFadd{for subsequent inclusion }\DIFaddend in the COSMOS \DIFdelbegin \DIFdel{field.
%DIF < , the target region of HSC survey.
}\DIFdelend \DIFaddbegin \DIFadd{catalog.
}\DIFaddend 
...
%DIF < As with the case of the simulated data, the classifier with absolute magnitudes and normalized flux provides the best classification results for PLAsTiCC data.
The AUC values for the ROC curve and the precision-recall curve are 0.996 and 0.995, respectively.
%DIF < , and these curves in this model are shown in figure\ \ref{fig:plasticc_2class_ROC} and figure\ \ref{fig:plasticc_2class_PreRec}.
...
\begin{itemize}
\item 54\% (58/107) of \DIFdelbegin \DIFdel{them  }\DIFdelend \DIFaddbegin \DIFadd{SNe }\DIFaddend were ``incomplete events'' that did not include the peak phase of the SN (the period \DIFaddbegin \DIFadd{(observed frame) }\DIFaddend of 10 days before and 20 days after the peak\DIFdelbegin \DIFdel{in the observed frame}\DIFdelend ) in the photometric data, whereas they only constitute 38\% of all events.
...
\item In more than half of the remaining 35 events, Ibc was misclassified as either Ia or II.
\end{itemize}
Figure\ \ref{fig:misclass_rate_3class} shows the accuracy against redshift for the PLAsTiCC dataset.
The accuracy for Ibc is lower than that of the other classes; in particular,
it is greatly reduced at redshifts beyond 1.0, and also decreased at redshift z of 0.1 to 0.2.
...
%DIF < Our DNN model requires exactly the same number of data points as an input. However, since the observing schedules (epochs) for each SN are common to some extent in each layer, and here we divide our dataset into five cases based on the number of photometric data points.
%
...
%DIF < in all of the schedule patterns from Case 0 to Case 4, and there is no new pattern common to them.
%
%v190522
\begin{table}[htbp]
...
%DIF < The performance for each input of binary classification are summarized in table\ \ref{tab:h2_validation},\ref{tab:h2_test_gold} and \ref{tab:h2_test_all}.
%DIF < The performance for each input of binary classification are summarized in table\ \ref{tab:h2_AUC}.
...
%DIF < Figure\ \ref{fig:h2_validation} show ROC curves and precision-recall curves in the best performing classifier for the validation set.

For the test dataset, the classification performance was verified using 1332 HSC SNe (1256 with redshift) labeled by the SALT2 \DIFdelbegin \DIFdel{light curve }\DIFdelend \DIFaddbegin \DIFadd{light-curve }\DIFaddend fitter \citep{guy2007,guy10b}, a conventional classification method.
The verification label for the HSC SNe conforms to that reported \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{previously }\DIFaddend \citet{yasuda19a}, which defines SN~Ia as SNe that satisfy all four of the following criteria for the SALT2 fitting results:
...
(4)number of degrees of freedom (dof) greater than or equal to five.
...
This performance evaluation was conducted by using 428 Ia and 904 non-Ia SNe classified by our machine.
%DIF < The numbers of labeled Ia, Ia? and non Ia are 428, 251 and 904 (409, and 847, with redshift), respectively.
%DIF < Those with the number of dof less than 5 are labeled as 'unclassified', which is not classified by SALT2 fit, and remaining 21 SNe are labeled 'fail' which does not even fit. 

We also extracted 441 ``light-curve verified SNe'' \DIFdelbegin \DIFdel{that have photometric information }\DIFdelend \DIFaddbegin \DIFadd{for which spec-z and photometric information were available }\DIFaddend before and after their peak\DIFdelbegin \DIFdel{and for which spec-z are available}\DIFdelend , and verified their classification results.
...
The best performing classifier obtained the same classification results as the conventional method for 84.2\% of 1256 labeled SNe, which is 91.8\% accurate for the 441 \DIFdelbegin \DIFdel{light curve }\DIFdelend \DIFaddbegin \DIFadd{light-curve }\DIFaddend verified SNe.
%DIF < The final classification result of the HSC SNe is the combination of outputs from the classifier using the absolute magnitude for those with redshift information, and for that using the magnitude for those without redshift information.
%DIF < The numbers of SNe with Ia probabilities above 0.5, 0.7 and 0.9 are 530, 386 and 170 out of 1332, respectively.
%DIF < The numbers of SNe with Ia probabilities above 0.5, 0.7 and 0.9 are 613, 437 and 186 out of 1540, respectively. (in old ver.)
%
%
%
%DIF < 
...
%DIF < Recall curves in two-class classification of light curve verified HSC SNe. 
  %DIF < The input to the classifier is absolute magnitude and normalized flux.
  %DIF < Each colored line represents the performance for each of the five classifiers with different input cases, and the that for all of their outputs.
}%
...
%DIF < because we are currently labeling more than three classes for all SNe detected in HSC-SSP Transient Survey using conventional light curve fitting.
%DIF < It is necessary to write the reason why there is no answer of multi-class classification.
%DIF < 
%DIF <  Classification performance of each model (3class, HSC, validation)
...
%DIF < Figure \ref{fig:h3_class_ratio} shows the predicted class ratio.
%DIF < Our machine classifies 1812 SN classes with a ratio of SN~Ia:Ibc:II=0.43:0.06:0.51, which is an integrated value throughout the redshift and includes incomplete sampling of faint SNe.

%
%
%DIF < %However, it should be noted that this ratio is calculated including misclassificaion.
%DIF < Table\  summarizes the classification results of all the actual SN.
%
%DIF < Need to write that 'ALL' accuracy is weighted
%DIF < 
%3class Validation
%Ver. 1.2.1
\begin{table}[htbp]
...
\footnotemark[$\dagger$] %DIF < Accuracy for samples extracted from each case according to the number ratio in the actual HSC dataset.
...
%DIF < The classification performance is low if only the initial rising phase before the SN peek is included in the input photometric data, but the accuracy increases to 78\% when we input two weeks of SN data from the first detection.
The classification accuracy is 78.1\% for the first two weeks of data, and after one month it increases to 82.7\%.
%DIF < Since the observation interval in the HSC survey is not constant, the number of inputs is different even for SNe with the same detected period.
%DIF < Due to this fact, there is a variation in the average input dimension number of samples in each phase, which causes a fluctuation in accuracy.
%DIF < However, due to events whose probabilities fluctuate with the progress of the SN phase illustrated in Figure \ref{fig:lcps}, there are some cases where the accuracy slightly decreases even if the period until classification becomes long.
%DIF < In addition, considering the cumulative number of SNe classified as Ia with high probability from six months data, it is estimated that our classifier can classify at least four or more Ia SNe candidate to follow-up in one month observation of the Ultra-deep area.
...
\begin{figure}[htbp]
  ...
  %DIF < Average number of observation points with red lines.
  }%
...
%DIF < IT changed 200710
%DIF < For each of 300 SNe labeled Ia and non Ia with SALT2 fitting, we extracted its Ia probability for each duration from the various classification results, and plotted its transition as figure\ \ref{fig:visualized_Ia_prob}.
%DIF < Although the Ia probability fluctuates greatly in the earliest phase, it begins to divide in about two weeks, and it can be clearly separated in one month.
%DIF < IT changed 200717
...
%DIF < Although the Ia probability fluctuates greatly and the cumulative ratio is less than 60\% in the earliest phase, it starts to split in about 2 weeks, and about 1 month after the detection, 80\% of supernovae are correctly classified. The initial cumulative rate is lower than the accuracy in Figure \ref{fig:n_observations_SNphase}, due to the SNe that are correctly classified at the beginning but eventually switch to the wrong type.
\DIFdelbegin \DIFdel{Lastly, we studied the transition of the Ia probability output by the classifier along the SN phase.
Each time the number of input epochs increases, the Ia probability, i.e., the output of the classifier, is updated.
In figure }\DIFdelend \DIFaddbegin \DIFadd{In Figure }\DIFaddend \ref{fig:visualized_Ia_prob} (Upper panel), we plotted the Ia probability at each epoch for each of 300 SNe labeled Ia and non-Ia based on the SALT2 fitting. The use of this result enabled us to measure the last epoch at which the correct type is classified, i.e., the epoch after which no further change in the classification occurs, for each SN. The lower panel shows the cumulative ratio for the epoch. The figure shows that the classification performance improves with time and that \DIFaddbegin \DIFadd{approximately }\DIFaddend 80\% of supernovae are correctly classified approximately 30 days after their first detection. In this figure, the initial cumulative ratio is lower than the accuracy shown in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig:n_observations_SNphase} because certain SNe that are initially correctly classified could ultimately be misclassified as the wrong type.

%
%DIF < Figure \ref{fig:HSTIaprob} shows the transitions of Ia probability along the days from the peak for 16 SNe included in Case 0 data, among 26 SNe selected as HST targets in the HSC survey, and the average of these transitions.
...
\begin{figure}[htbp]
  ...
  \end{center}
  \caption{%DIF < IT changed 200710
  %DIF < Transition of Ia probability with increasing elapsed time of input SNe.
  %DIF < Each line corresponds to one of SNe.
  %DIF < The difference in color indicates the difference in label, with red being Ia and blue being non Ia.
  %DIF > 
...
%DIF < \subsection{Simulated data versus observed data}
%
%DIF < In this paper we not only evaluate our classifier with simulated data, but also apply it to actual HSC survey data to evaluate classification performance.
...
%DIF < As with the classification of the PLAsTiCC data, many misclassified incomplete events, which should be only 27\% in all classified SNe, suggest that they reduce the performance of the classifier.
...
%DIF < , as with classification of PLAsTiCC data.
%DIF < However, the misclassification rate of incomplete events is clearly increased to 7\% (93/1256) in HSC data compared to 3\% (58/2297) in PLAsTiCC data.
...
%DIF < The presence of anomalous events also reduces the performance of actual survey classifications.
%
%DIF < Of course, the labeling of the actual data class by the conventional method includes uncertainty, but the difference in the results should be minimized by making the learning data more similar to the actual data.
%
%
%DIF < \subsection{Comparison with the best classifier in PLAsTiCC}
...
%DIF < only for the compared SNe, our classifier is about 6\% better in total accuracy than the PLAsTiCC 1st classifier and 6\% better in Ia accuracy.
%DIF < Considering that our classifier only uses the middle one year data out of the original three year data as input, it classifies PLAsTiCC data with better accuracy with less data than that of PLAsTiCC 1st.
...
%DIF < IT added 200709, Canceled 200717
%DIF < Furthermore, as described in section \ref{sec:comparison}, our classifier achieves a comparable result with the best model of the Kaggle competition, which interpolates the data and extracts the features.
%
%DIF < To classify all objects with our method, we need to prepare multiple classifiers.
%DIF < In order to support ultra-wide-range surveys such as LSST, it is necessary to input features that are less affected by the magnitude of the cadence for each transient, and to add processing to interpolate missing values.
%
%DIF < 
...
%DIF < Cosmological origin of elements depends of the cosmic history of SNe.
%DIF < show solar abundance 
%
%
%
\section{Conclusions}
...
%DIF < \item 
%DIF < \item 
%DIF < \item our new method(2DGP+LGBM)
%DIF < \end{itemize}
%
...
%DIF < Furthermore, our proposed method is easy to apply to estimation models such as redshift because of simple input. The application of our model to redshift estimation is useful for the selection of host galaxies by comparing with the photo-z information of galaxies.
%
\begin{ack}
...
%DIF < IT, NS and NY acknowledge financial support from JST CREST (JPMHCR1414).
%DIF < This work is supported by JST CREST Grant Number JPMHCR1414, Japan.
%DIF < NY is supported by the Grant-in-Aid for Scientific Research program of MEXT (18H04345). NS is supported by JSPS (18K03696).
%DIF < MT is supported by the Grant-in-Aid for Scientific Research program of MEXT (17H06363) and JSPS (16H02183,19H00694).
...
%DIF < \subsection{Estimating Redshift}
...
%DIF < There exists cases that we may not have a photometric redshift (hostless supernvoae) or photometric redshift is not accurate.
%DIF < For these cases, we can estimate the redshift $z$ of a SN from the observed brightness.
%DIF < We cannot use absolute magnitude to estimate the redshift value because redshift value is required to calculate absolute magnitude.
%DIF < Therefore, we used magnitude instead of absolute magnitude as the input of the DNN.
...
%DIF <  $\mathrm{SE} \left(z, y\right) = \frac{1}{2} \left| z - y \right|^2 $
